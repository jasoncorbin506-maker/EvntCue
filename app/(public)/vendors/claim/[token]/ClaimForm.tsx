 ...
 * useActionState wrapper around the server action, conditional render
 * based on signed-in state, error surface on action failure.
 *
 * Per master spec §75 + Vndr Locked architecture, this form is the
 * second-impression moment after the warm-intro email. The accent error
 * color uses the coral token #E8622A (Vndr accent per spec §18) — same
 * literal hex as Venu's error surface, repurposed here as a brand match.
 */
  ...
  return (
    <main className={s.phone}>
      <header className={s.formChrome}>
        ...
        <div className={s.formStep}>Claim your profile</div>
      </header>

      <div className={s.formBody}>
        <h1 className={s.formH}>
          Welcome, <i>{vendorDisplayName}</i>
        </h1>
        <p className={s.formSub}>
          {isSignedIn ? (
            <>
              We&apos;ve pre-built your dashboard
              {vendorCity ? ` for ${vendorCity}` : ""}. One tap to add it to your
              account ({signedInEmail}).
            </>
          ) : (
            <>
              We&apos;ve pre-built your dashboard
              {vendorCity ? ` for ${vendorCity}` : ""}. Set a password to take
              ownership — you&apos;ll be inside in seconds.
            </>
          )}
        </p>
        ...
        <form action={formAction} noValidate>
          ...
          <button type="submit" className={s.formSubmit} disabled={pending}>
            {pending
              ? "…"
              : isSignedIn
                ? `Add ${vendorDisplayName} to my account`
                : "Claim my dashboard"}
          </button>
        </form>
        ...
        <div className={s.formNote} style={{ marginTop: 14 }}>
          ...
          <div className={s.formNoteTxt}>
            <b>Not the right business?</b> Don&apos;t use this link — it&apos;s
            tied to one profile. Email{" "}
