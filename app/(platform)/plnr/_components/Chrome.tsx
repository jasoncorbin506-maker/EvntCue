...
/**
 * Top chrome for the Plnr portal. Planner mark + name + role label on the
 * left, optional right slot. Mirrors the catr/venu/vndr Chrome pattern — the
 * mark is decorative (NOT a tap target); a back arrow only renders on detail
 * routes via `backHref`.
 */
...
export function Chrome({ plannerName, roleLabel, right, backHref }: ChromeProps) {
  return (
    <header className={s.chrome}>
      <div className={s.chromeL}>
        ...
        <div className={s.chromeName} style={backHref ? { marginLeft: 8 } : undefined}>
          <div className={s.chromeCaterer}>{plannerName}</div>
